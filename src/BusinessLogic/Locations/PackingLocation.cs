...
namespace PharmaProject.BusinessLogic.Locations
{
    public class PackingLocation : BaseLocation
    {
        ...
        protected override void InitScripts()
        {
            ...
            csd1AutoLoad = MakeConditionalMacro($"Auto load CSD:1, Loc:{LocationNumber}").AddStatement(conditional).AddStatement(scripts.LoadNormal);
        }
        ...
        private void HandleCsd2()
        {
            ...
            switch (csd2.State)
            {
                ...
                case SEGMENT_STATE.OCCUPIED:
                    DispatchCSD2(route?.Destination ?? DESTINATION.TBD);
                    break;
            }
        }
